<?php
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "SELECT * FROM users WHERE (BINARY username = '$user' OR BINARY tenant_email = '$user' OR BINARY tenant_phone_no = '$user') AND BINARY `password` = '$pass'";
    $result = $con->query($query);

    if ($result->num_rows >= 1) {
      $row = $result->fetch_assoc();
      $t_cookie = $row['tenant_name'] ?? $user;
      setcookie("user", $user);
      $output=true;
    ...
    else
    {
      ...
      if ($result->num_rows >= 1) {
        ...
        if ($status == "Rejected") {
          // echo '<script>';
          // echo 'document.getElementById("error").innerHTML = "Sorry, Your Profile has been Rejected by Admin Please re-sign up";';
          // echo 'setTimeout(function(){ window.location.href = "user.php"; }, 3000);';
          // echo '</script>';
          $rejected = array(
          ...
          // echo '<script>';
          // echo 'document.getElementById("error").innerHTML = "Sorry, still pending";';
          // echo 'setTimeout(function(){ window.location.href = "user.php"; }, 3000);';
          // echo '</script>';
          $pending = array(
            ...
            'icon' => 'wait-icon',
            'errortitle' => "Sorry",
            'errorMsg' => "Your Profile Validation Still in Pending."
          );
          header('Content-Type: application/json');
        ...
        // echo '<script>';
        // echo 'document.getElementById("error").innerHTML = "Username or password must be wrong or Please Sign Up First";';
        // echo 'setTimeout(function(){ window.location.href = "user.php"; }, 3000);';
        // echo '</script>';
        $error = array(
          ...
          'icon' => 'error-icon',
          'errortitle' => "You sure!",
          'errorMsg' => "Username or password must be wrong or Please Sign Up First",
        );
        header('Content-Type: application/json');
        ...
        <div class="text-box text-box2">
          ...
          <form class="login-form signup-form"  id="signup-form">
            ...
            <div class="pass-pack">
              ...
              <ion-icon class="hide" name="eye-off-outline"></ion-icon>
            </div>
            <div class="btn-pack">
